...
df <- read.csv("/Users/jmadu1/Documents/LSE\ Accelerator/Course\ 3/LSE_DA301_assignment_files_new/turtle_reviews_cleaned.csv", header=T)
...
# Histogram to view the distribution of loyalty points
...
# Scatterplot to see the relationship between loyalty points and remuneration
ggplot(df, aes(x = remuneration, y = loyalty_points)) +
  geom_point(alpha = 0.6) +
  labs(title = "Scatterplot of Remuneration vs Loyalty Points", x = "Remuneration", y = "Loyalty Points")

# Barplot to visualise the relationship between loyalty points and gender
ggplot(df, aes(x = gender, y = loyalty_points)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  coord_flip() +
  labs(title = "Barplot of Gender vs Loyalty Points", x = "gender", y = "loyalty_points")
...
# Analyzing the distribution of loyalty points
ggplot(df, aes(x = loyalty_points)) +
  geom_histogram(bins = 30, fill = "blue", color = "black") +
  labs(title = "Histogram of Loyalty Points", x = "Loyalty Points", y = "Frequency")

# Check for outliers and general spread
boxplot(df$loyalty_points, main = "Boxplot for Loyalty Points")

# Correlation matrix to check for multicollinearity
cor(df[c("age", "remuneration", "spending_score", "product", "loyalty_points")])

# Building the model
model <- lm(loyalty_points ~ remuneration + spending_score, data = df)
summary(model)
...
new_data <- data.frame(remuneration = c(13.30, 41.80), spending_score = c(50, 10))
